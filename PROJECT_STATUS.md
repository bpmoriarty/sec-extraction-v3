...
**NEW FINDING — `no_balance_sheet_title` IS FALSE ON ~84% OF THE FILINGS IT FLAGS (not yet
fixed).** Probing all 208 flagged filings shows the balance-sheet heading is **inside the block we
already extract** in **175** of them; only **4** are genuinely missing an A&L title (29 have one
outside every block). Cause: the block's start anchor is sometimes a footnote *reference* rather
than a heading — Pioneer's Level-3 note says "…included in the realized gain (loss) from
investments on the **Statement of Operations**", and the `_REFERENCE_CUES` look-behind is only 48
characters, so "included in" is out of reach. That false Operations anchor sits ~1,300 chars
*before* the real balance-sheet title, and the balance sheet is only ever searched **backward**
from `ops.start`, so a title that follows it can never be credited. Spread across ~40 filers (AIP
15+14, Pioneer 13, SkyBridge 9, Aspiriant 8, Agility 7, Priority Income 7, Cypress Creek 6+6…).
**Consequence is diagnostic, not data loss** — the balance sheet is in the serialized text either
way, so the LLM will still read it and C1 will still validate; what is wrong is the reported
balance-sheet coverage (96.9%, understated) and 175 filings needlessly routed to the review queue.
Two candidate fixes: credit an A&L heading found *inside* the block span (low risk, diagnostic
only, but could over-credit a note that merely names the statement), or widen reference-cue
detection (more principled, riskier — it could start rejecting real headings). **Deliberately left
for a separate change** so it cannot be conflated with the ASCII-table fix above.
**STILL OPEN (all free to investigate, no API key):** (1) **`no_balance_sheet_title` false on 175
filings** — the finding immediately above, the largest remaining item; (2) **62 filings flagged
`no_anchors`** — undiagnosed, mostly large documents hitting the truncated whole-doc fallback;
...
**Next: the `no_balance_sheet_title` false-flag fix** (175 filings, evidence gathered — see above),
then **M2** (`ncsr_series.py`, the multi-series slicer) over the 428 remaining multi-block filings.
Still no API key needed.
**Last Session: 2026-08-05 (session 19 — Pioneer ILS resolved via legacy-ASCII-table serializer;
gate 98.5% → 98.8%; `no_balance_sheet_title` false-flag class identified)**
