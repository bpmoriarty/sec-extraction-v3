...
**Next: M2** (`ncsr_series.py`, the multi-series slicer) over the 428 remaining multi-block filings.
Optionally first: the ACAP-style balance-sheet-after-statements recovery (52 filings). Still no API
key needed.
**Last Session: 2026-08-05 (session 19 — Pioneer ILS resolved via legacy-ASCII-table serializer,
gate 98.5% → 98.8%; `no_balance_sheet_title` false flags fixed, 217 → 141, balance-sheet coverage
96.9% → 98.4%, zero content change)**
