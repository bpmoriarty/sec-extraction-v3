...
**STILL OPEN (all free to investigate, no API key):** (1) **Pioneer ILS Interval Fund, 9 filings**
— a genuine location failure: real statements sit at 22–25% of the document but only the contents
page is found; (2) **31 filings flagged `no_anchors`** — undiagnosed, mostly large documents hitting
the truncated whole-doc fallback; (3) ~7 table-less filings still failing (BlueBay Destra, Pender,
Destra Multi, Aspiriant) — their narrative still yields too few financial values; (4) 17 singleton
misses, deliberately out of scope under the ≥3-filings rule; (5) CNR's one residual restatement
block. **Recommended sequencing: finish this miss pass before M2**, because the M4 gold sample
gates the entire spend and hand-verifying 25 filings against a locator/serializer still in motion
would waste the most expensive human step in the plan.
**Next: M2** (`ncsr_series.py`, the multi-series slicer) over the 428 remaining multi-block
filings, after the miss pass. Still no API key needed.
**Last Session: 2026-08-03 (session 18 — M1 locator + inventory built, gate passed on smoke test)**
